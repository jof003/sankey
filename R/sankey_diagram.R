...
sankey_diagram <- function(df, include_borders = TRUE) {
  ...
  ggplot2::ggplot(df_v2, ggplot2::aes(x = df_v2$year,
    ...
    geom_text(ggplot2::aes(label = df_v2$value), stat = "stratum",colour = "white", size = 3, fontface = "bold", decreasing = FALSE) +
    # Add the title.
    ...
    geom_text(ggplot2::aes(label = ifelse(df_v2$year == "1990", df_v2$risk_factor," ")), stat = "stratum", hjust = 0, nudge_x = -1.75, color = "black", size = 3,  decreasing = FALSE) +
